...
Diese Prüferliste muss einmalig in den Einstellungen vorgenommen werden und wird dann beim Verlassen des Programms (wahlweise) abgespeichert und steht dann nächstes mal gleich zur Verfügung (s.~Abschnitt~\ref{sec:einstellungen} auf Seite~\pageref{sec:einstellungen}).
...
Links unten in Abbildung~\ref{fig:pihk} findet man Angaben zur Prüfung, wie Datum der Prüfung (\circled{3}) und Name (\circled{3}) und Nummer (\circled{2}) des Prüflings.
